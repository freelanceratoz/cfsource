...
<div class="main-admn-usr-lst js-response">
	<div class="bg-primary row">		
		<ul class="list-inline sec-1 navbar-btn">
			...
			<li>
				<?php echo $this->Html->link('<span class="img-circle img-thumbnail bg-sucess img-wdt center-block text-center opn-i-usr">'.$this->Html->cInt($pending + $approved,false).'</span><span>' .__l('All'). '</span>', array('controller'=>'equity_project_categories','action'=>'index'), array('class' => 'text-center','escape' => false));?>
			</li>
		</ul>		
	</div>
	<div class="clearfix">		
		...
		<?php echo $this->Form->create('EquityProjectCategory', array('action' => 'update','class'=>'clearfix js-shift-click js-no-pjax')); ?>
			...
			<table class="table table-striped">
				...
				<tbody class="h5">	
					...
					foreach ($projectCategories as $projectCategory):
							 if($projectCategory['EquityProjectCategory']['is_approved'] == '1')  :
					...
					?>
						<tr <?php echo $disabled; ?>>
							<td class="text-center">
								<?php
								echo $this->Form->input('EquityProjectCategory.'.$projectCategory['EquityProjectCategory']['id'].'.id',array('type' => 'checkbox', 'id' => "admin_checkbox_".$projectCategory['EquityProjectCategory']['id'],'label' => '' , 'class' => $status_class.' js-checkbox-list'));?>
							</td>
							<td class="text-center">
								<div class="dropdown">
									...
									<ul class="dropdown-menu">
										<li>
										<?php echo $this->Html->link('<i class="fa fa-pencil-square-o fa-fw"></i>'.__l('Edit'), array( 'action'=>'edit', $projectCategory['EquityProjectCategory']['id']), array('class' => 'js-edit','escape'=>false, 'title' => __l('Edit')));?>
										</li>
										<li>
										<?php echo $this->Html->link('<i class="fa fa-times fa-fw"></i>'.__l('Delete'), Router::url(array('action'=>'delete',$projectCategory['EquityProjectCategory']['id']),true).'?r='.$this->request->url, array('class' => 'js-confirm ', 'escape'=>false,'title' => __l('Delete')));?>
										</li>
										<?php echo $this->Layout->adminRowActions($projectCategory['EquityProjectCategory']['id']);  ?>
									</ul>
							  </div>
							</td>
							<td class="text-left"><?php echo $this->Html->cText($projectCategory['EquityProjectCategory']['name']);?></td>
							<td class="text-center"><?php echo $this->Html->link($this->Html->cInt($projectCategory['EquityProjectCategory']['equity_count']), array('controller' => 'equities', 'action' => 'index', 'project_category_id' => $projectCategory['EquityProjectCategory']['id']), array('escape' => false));?></td>
						</tr>
						<?php
						endforeach;
						else:
						?>
						<tr>
							<td colspan="7" class="text-center text-danger"><i class="fa fa-exclamation-triangle fa-fw"></i> <?php echo sprintf(__l('No %s available'), sprintf(__l('%s %s Categories'), Configure::read('project.alt_name_for_equity_singular_caps'), Configure::read('project.alt_name_for_project_singular_caps')));?></td>
						</tr>
						<?php
						endif;
						?>
