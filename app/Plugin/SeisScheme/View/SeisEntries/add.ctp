			...
			<div class="container clearfix">
				<div class="equity-form">
					<div class="clearfix">
					  ...
					  <?php
					  echo $this->Form->input('company_name',array('label' => __l('Company Name'),'placeholder'=>__l('Enter the Company name')));
					  echo $this->Form->input('number_of_employees',array('label' => __l('Number of Employees'),'placeholder'=> __l('Enter the number'))); ?>
					   <div class="input date-time clearfix">
						  <div class="js-datetime required">
						  <div class="js-cake-date">
							<?php echo $this->Form->input('year_of_founding', array('label' => __l('Year Founded'), 'type' => 'date', 'orderYear' => 'asc', 'maxYear' => date('Y'), 'minYear' => date('Y') - 200, 'div' => false, 'empty' => __l('Please Select') , 'info' => __l('The year your company was founded'),'placeholder'=> __l('No Date Set'))); ?>
						  </div>
						  </div>
					   </div>
					   <?php echo $this->Form->input('total_asset',array('label' => __l('Total Asset ($)'),'placeholder'=> __l('Enter amount'))); ?>
