...
class DonateChartsController extends ChartsController
{
    public $name = 'DonateCharts';
    ...
    public function chart_projects() 
    {
        ...
        if ($this->Auth->user('role_id') == ConstUserTypes::Admin) {
            ...
            if (!empty($city_filter_id)) {
                $project_cities = $this->Donate->User->UserProfile->City->find('first', array(
                    'conditions' => array(
                ...
                foreach($project_cities['Donate'] as $project_city) {
                    $city_project_id[] = $project_city['id'];
                }
                $conditions['Donate.id'] = $city_project_id;
            }
        }
        $project_model_data['Pending'] = array(
            'display' => __l('Pending') ,
            'conditions' => array_merge(array(
                'Donate.donate_project_status_id' => ConstDonateProjectStatus::Pending
            ) , $conditions) ,
        );
        $project_model_data['OpenForDonating'] = array(
		...
		if (isPluginEnabled('Idea')) {
			$project_model_data['OpenForIdea'] = array(
        ...
        $chart_projects_data = $this->_setLineData($select_var, $project_model_data, 'Donate.Donate', 'Donate', $common_conditions);
        $project_fund_model_data['Paid'] = array(
            'display' => __l('Paid') ,
            'conditions' => array(
        ...
        $chart_project_funds_data = $this->_setLineData($select_var, $project_fund_model_data, 'Projects.ProjectFund', 'ProjectFund', $common_conditions);
        $this->set('chart_projects_data', $chart_projects_data);
        $this->set('chart_projects_periods', $project_model_data);
        $this->set('chart_project_funds_data', $chart_project_funds_data);
        $this->set('chart_project_funds_periods', $project_fund_model_data);
