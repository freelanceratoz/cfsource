...
<div class="col-xs-12">	
	...
	?>
		<div class='<?php echo $fund_class;?> hide'> <?php //after login project fund?>
			<a href="<?php echo Router::url(array('controller' => 'project_funds', 'action' => 'add', $project['Project']['id']), true); ?>" class="btn btn-success btn-lg text-uppercase navbar-btn fa-inverse js-tooltip" title="<?php echo sprintf(__l('%s'), Configure::read('project.alt_name_for_donate_singular_caps')); ?>"><?php echo sprintf(__l('%s'), Configure::read('project.alt_name_for_donate_singular_caps')); ?></a>
		</div>	
	<?php } else { ?>
		<div class='alof-<?php echo $this->Html->cInt($project['Project']['id'], false);?> hide'>  <?php //after login project owner fund?>
			<span class="disabled btn btn-success btn-lg text-uppercase navbar-btn fa-inverse js-tooltip" title="<?php echo sprintf(__l('Disabled. Reason: You can\'t %s your own %s.'), Configure::read('project.alt_name_for_donate_singular_small'), Configure::read('project.alt_name_for_project_singular_small')); ?>"><?php echo Configure::read('project.alt_name_for_donate_singular_caps');?></span>
		</div>
	<?php } ?>
	...
	<div class='ablfc-<?php echo $this->Html->cInt($project['Project']['id'], false);?> hide'> <?php //after or before login project fund closed?>
	 <span class="disabled btn btn-success btn-lg text-uppercase navbar-btn fa-inverse js-tooltip" title="<?php echo sprintf(__l('Disabled. Reason: %s.'),  $reason); ?>"><?php echo sprintf(__l('%s'), Configure::read('project.alt_name_for_donate_singular_caps'));?></span>
	</div>

</div>
<?php } else { ?>
<?php if($project['Donate']['donate_project_status_id'] == ConstDonateProjectStatus::OpenForDonating): ?>
<div class="col-xs-12 donate">
...
<?php else :
  ...
  $reason =  $status_response->data['response'];
?>
<div class="col-xs-12 donate"><span class="disabled btn btn-success btn-lg text-uppercase navbar-btn fa-inverse js-tooltip" title="<?php echo sprintf(__l('Disabled. Reason: %s.'),  $reason); ?>"><?php echo sprintf(__l('%s'), Configure::read('project.alt_name_for_donate_singular_caps'));?></span></div>
<?php endif; ?>
<?php }?>